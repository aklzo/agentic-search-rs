...
## 1.5 Ollama は既定でプロンプトを 4K トークンに無言で切り詰める(最重要級)

12B モデルの実験中、評価呼び出しが `{` 一文字だけ返して死ぬ事象から発覚した。**Ollama の既定コンテキスト長(`num_ctx`)は 4096 トークン**で、超過した入力は**エラーにならず先頭側から黙って捨てられる**。

- 本ツールの評価者は最大 12,000 字のダイジェストを渡すため確実に超過しており、**修正までの全ローカル実験は「途中までしか読んでいない評価」で動いていた**。3B 実験の不可解な採点(鮮度0など)の一因だった可能性が高い
- 対策: `/api/chat` の `options.num_ctx` を明示する(本実装は 16384。ユニットテストで固定)
- **教訓**: 「動いているように見えるが入力が読まれていない」型の障害は検知が極めて難しい。プロンプトサイズがモデル・サーバーの既定値を超えうる設計なら、コンテキスト長は必ず明示する
- 連鎖した二次障害も記録しておく: num_ctx を直すと全文を読むようになり、今度は 12B の prefill が**クライアントのタイムアウト(180秒)を正常動作のまま超過**した。ローカル推論のタイムアウトは API の常識(数十秒〜3分)で設定してはいけない(本実装は Ollama のみ 900 秒)
...
- **並列処理のテストは「タイミング」でなく「同時実行ピーク」を見る**。`sleep` で速さを測る時間ベースのテストは flaky なので、フェッチャに `AtomicUsize` の in-flight カウンタと `fetch_max` でピークを記録させ、並列度4でピーク>1・並列度1でピーク=1 を検証した(`gatherer.rs`)。

## 10. `&mut store` を持つループの並列化 — Mutex を使わずに済ませる

収集ループ(`for hit in hits { process(&mut store) }`)を並列化したとき、素直にやると「複数の非同期タスクが `&mut KnowledgeStore` を共有 → `Arc<Mutex<Store>>`」に行きがちだが、**3段に分けるとロックが要らない**:

1. **選択(逐次)**: `&mut store` を使う処理(訪問済みマーク・上限カウント)はループ前にまとめて済ませ、対象 `Vec<Hit>` を確定する
2. **重い処理(並列)**: store に触れない純粋関数 `extract_page(hit) -> Result<Vec<Finding>>` に切り出し、`futures::stream::iter(...).buffered(n)` で同時実行
3. **マージ(逐次)**: 結果 `Vec` を順に `store.add_finding` で取り込む
